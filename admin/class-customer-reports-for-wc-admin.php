...
class Customer_Reports_For_Wc_Admin {
	...
	public function customer_list_callback() {
		echo '<div class="wrap"><h1>' . esc_html__( 'Customers', 'customer-reports-for-wc' ) . '</h1></div>';
		...
		if ( ! empty( $customers ) && ! empty( $orders ) ) {
			...
			foreach ( $customers as $_cuskey => $_cusval ) {
				...
				if ( empty( $lastordertext ) ) {
					$lastordertext = __( 'No Orders', 'customer-reports-for-wc' );
				}

				if ( '' === $order_count ) {
					$orderstotal = '<div class="usercal" data-user="' . esc_attr( $customer_id ) . '"><span style="vertical-align: top;">' . esc_html__( 'Calculating', 'customer-reports-for-wc' ) . '</span><img src="' . plugin_dir_url( __FILE__ ) . '/images/spinner.gif"></div>';
				...
				if ( '' === $total_spent ) {
					$ordersspend = '<div class="usercal" data-user="' . esc_attr( $customer_id ) . '"><span style="vertical-align: top;">' . esc_html__( 'Calculating', 'customer-reports-for-wc' ) . '</span><img src="' . plugin_dir_url( __FILE__ ) . '/images/spinner.gif"></div>';
				} else {
				...
				echo '<tr>';
					...
					echo '<td>' . esc_html( date( 'j F Y', strtotime( $_cusval->data->user_registered ) ) ) . '</td>';
					echo '<td>' . esc_html( $average ) . '</td>';
					echo '<td>' . $lastordertext . '</td>';
					echo '<td>' . $orderstotal . '</td>';
					echo '<td>' . $ordersspend . '</td>';
				echo '</tr>';
			}
			echo '<tbody>';
			echo '</table>';
			...
			echo '<div class="wrap"><h3>' . esc_html__( 'No Customers/Orders Found!', 'customer-reports-for-wc' ) . '</h3></div>';
	...
	public function get_order_average( $firstorderdate, $lastorderdate, $totalcusorders, $customer_id ) {
		...
		if ( $diff > 0 ) {
			$countco = $totalcusorders - 1;
						update_user_meta( $customer_id, 'customer_average', round( $diff / $countco ) );
			return __( 'Every ' . round( $diff / $countco ) . ' days', 'customer-reports-for-wc' );
		} else {
						update_user_meta( $customer_id, 'customer_average', 0 );
			return __( 'No Average', 'customer-reports-for-wc' );
		}
	}
	...
	public function enqueue_styles() {
		...
		if ( $screen->id == 'toplevel_page_crfwc' ) {
			...
			wp_enqueue_style( 'datatables-bootstrapmin-css', plugin_dir_url( __FILE__ ) . 'css/dataTables.bootstrap4.min.css' );
			wp_enqueue_style( 'buttons.dataTables-css', plugin_dir_url( __FILE__ ) . 'css/buttons.dataTables.min.css' );
		}
	}
	...
	public function enqueue_scripts() {
		...
		if ( $screen->id == 'toplevel_page_crfwc' ) {
			...
			wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/customer-reports-for-wc-admin.js', array( 'jquery' ), $this->version, false );
		}
		$asset_file = plugin_dir_path( __FILE__ ) . 'js/build/crfwc-index.min.asset.php';

		if ( file_exists( $asset_file ) ) {
			$asset = include $asset_file;
			// wp_enqueue_style( 'jobplace-style', plugin_dir_url( __FILE__ ) . 'js/build/index.css' );

			wp_enqueue_script( 'chart-index-script', plugin_dir_url( __FILE__ ) . 'js/build/crfwc-index.min.js', $asset['dependencies'], time(), true );
		}
	}

	public function wporg_add_dashboard_widgets() {
		wp_add_dashboard_widget(
			'crfwc-dashboard-widget-1',                          // Widget slug.
			esc_html__( 'Chart for WooCommerce', 'customer-reports-for-wc' ), // Title.
			array( $this, 'chart_for_wc_cb' )                    // Display function.
		);
	}
	public function chart_for_wc_cb() {
		?>
			<div id="chart_wrapper" style="width: 500px; height: 500px;"></div>
		<?php
	}
}
